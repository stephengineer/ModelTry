...
def train(args):
    ...
    train_data = torchvision.datasets.CIFAR10(
        root="dataset", download=True, transform=transform, train=True
    )
    test_data = torchvision.datasets.CIFAR10(
        root="dataset", download=True, transform=transform, train=False
    )
            ...
            if (i + 1) % args.print_map_epoch == 0:  # 100次显示一次loss
                print(
                    "Epoch [{}/{}], Step [{}] Loss: {:.4f}".format(
                        epoch + 1, num_epochs, total_train_step, loss.item()
                    )
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--device_num", type=str, default="cpu", help="select GPU or cpu"
    )
    parser.add_argument(
        "--image_size",
        type=int,
        default=224,
        help="if crop img, img will be resized to the size",
    )
    parser.add_argument(
        "--batch_size", type=int, default=32, help="batch size, recommended 16"
    )
    parser.add_argument("--lr", type=float, default=0.001, help="learning rate")
    parser.add_argument(
        "--momentum", type=float, default=0.90, help="choice a float in 0-1"
    )
